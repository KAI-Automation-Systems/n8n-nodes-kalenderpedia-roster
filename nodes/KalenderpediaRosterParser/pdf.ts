...
// pdf-parse als CommonJS import (vermijdt TS2349)
...
try {
  ...
  (pdfjsLib as any).GlobalWorkerOptions.workerSrc =
    (workerSrc as any).default || workerSrc;
} catch {
...
async function extractWithPdfJs(pdfBuffer: Buffer): Promise<number[]> {
  try {
    ...
    throw new PdfParseError(
      `pdfjs-dist failed: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
...
export async function extractDaysWithSabine(
  pdfBuffer: Buffer
): Promise<number[]> {
  try {
